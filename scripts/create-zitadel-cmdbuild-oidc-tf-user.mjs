...
if (!['admin', 'editor', 'reader'].includes(role)) throw new Error('usage: create-zitadel-cmdbuild-oidc-tf-user.mjs <admin|editor|reader>');
...
const initialPassword = readFileSync(`secrets/zitadel_cmdbuild_oidc_tf_${role}_password`, 'utf8').trim();
...
try {
  ...
  if (await existing.count()) {
    ...
    console.log(JSON.stringify({ role, status: 'already_exists', user_id_hash: existingUserId.slice(-8), state_path: statePath(`${role}_user_id`) }));
    process.exit(0);
  }
