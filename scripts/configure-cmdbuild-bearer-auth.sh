...
placeholder='replace-with-cmdbuild-resource-project-id'
...
test "${CMDBUILD_BEARER_AUDIENCE}" = "${CMDBUILD_RESOURCE_AUDIENCE}"
test -n "${CMDBUILD_BEARER_USER_CLAIM}"
test -n "${CMDBUILD_BEARER_AUDIT_SINK_URL}"
test -r "${CMDBUILD_BEARER_AUDIT_HMAC_KEY_FILE}"
...
set_config org.cmdbuild.auth.bearer.auditSinkHmacKeyFile "$CMDBUILD_BEARER_AUDIT_HMAC_KEY_FILE"
